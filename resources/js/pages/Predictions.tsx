...
export default function Predictions({
  ...
  // Form handling (replaces manual useState)
  const { data, setData, post, processing } = useForm({
    program_id: filters.programId || '',
    academic_year: filters.academicYear || academicYears?.[0] || '',
                ...
                <form onSubmit={handlePredict} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm xl:col-span-1">
                  ...
                  <div className="flex flex-col gap-5">
                    ...
                    <button
                      type="submit"
                      disabled={processing}
                      className="mt-4 flex w-full items-center justify-center rounded-xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-70"
                    >
                      {processing ? 'Processing...' : 'Generate Prediction'}
                    </button>
                  </div>
                </form>
              </div>
